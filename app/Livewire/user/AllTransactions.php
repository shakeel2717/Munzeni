...
final class AllTransactions extends PowerGridComponent
{
    ...
    {
        return Transaction::query()->where('user_id', auth()->user()->id)->whereIn('type', $this->type)->latest();
    }
    ...
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ...
            ->addColumn('type_lower', fn (Transaction $model) => strtoupper(e($model->type)))

            ->addColumn('amount_format', function (Transaction $model) {
                return "$" . number_format($model->amount, 2);
            })
            ->addColumn('reference_lower', fn (Transaction $model) => strtoupper(e($model->reference)))
    ...
    {
        return [
            Column::make('Type', 'type_lower', 'type')
                ->sortable()
                ->searchable(),
            ...
            Column::make('Reference', 'reference_lower', 'reference')
                ->sortable()
                ->searchable(),
